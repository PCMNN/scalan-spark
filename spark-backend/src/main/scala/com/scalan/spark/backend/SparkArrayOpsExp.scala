...
trait SparkArrayOpsExp extends Variables with Transforming with VariablesExp { self: LmsBackendFacade =>
  ...
  case class ArrayRandomGaussianSparkLms[A:Manifest](a: Exp[Double], e: Exp[Double], xs: Exp[Array[Double]]) extends Def[Array[Double]] {
    val m = manifest[Double]
  }

  def array_randomGaussianSparkLms[A:Manifest](a: Rep[Double], e: Rep[Double], arr: Rep[Array[Double]]): Rep[Array[Double]] = ArrayRandomGaussianSparkLms(a, e, arr)

  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case ArrayRandomGaussianSparkLms(a, d, arr) => array_randomGaussianSparkLms(f(a), f(d), f(arr))(mtype(manifest[Double])).asInstanceOf[Exp[A]]
...
trait SparkScalaGenArrayOps extends ScalaGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case ds @ ArrayRandomGaussianSparkLms(a, d, arr) =>
