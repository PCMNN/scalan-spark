...
!!!("Cannot convert from SparkAbstractMatrix to SparkSparseMatrix: missing fields List(rddIdxs, rddVals, numColumns)")
  ...
  class SparkSparseMatrixIso[T](implicit elem: Elem[T])
    ...
    lazy val defaultRepTo: Rep[SparkSparseMatrix[T]] = SparkSparseMatrix(element[RDDCollection[Array[Int]]].defaultRepValue, element[RDDCollection[Array[T]]].defaultRepValue, 0)
  ...
  implicit case object SparkSparseMatrixCompanionElem extends CompanionElem[SparkSparseMatrixCompanionAbs] {
...
!!!("Cannot convert from SparkAbstractMatrix to SparkDenseMatrix: missing fields List(rddVals, numColumns)")
  ...
  class SparkDenseMatrixIso[T](implicit elem: Elem[T])
    ...
    lazy val defaultRepTo: Rep[SparkDenseMatrix[T]] = SparkDenseMatrix(element[RDDCollection[Array[T]]].defaultRepValue, 0)
  ...
  implicit case object SparkDenseMatrixCompanionElem extends CompanionElem[SparkDenseMatrixCompanionAbs] {
...
trait SparkMatricesExp extends SparkMatricesDsl with SparkDslExp {
  ...
  object SparkSparseMatrixMethods {
    ...
    object mapBy {
      def unapply(d: Def[_]): Option[(Rep[SparkSparseMatrix[T]], Rep[AbstractVector[T] => AbstractVector[R] @uncheckedVariance]) forSome {type T; type R}] = d match {
        case MethodCall(receiver, method, Seq(f, _*), _) if receiver.elem.isInstanceOf[SparkSparseMatrixElem[_]] && method.getName == "mapBy" =>
          Some((receiver, f)).asInstanceOf[Option[(Rep[SparkSparseMatrix[T]], Rep[AbstractVector[T] => AbstractVector[R] @uncheckedVariance]) forSome {type T; type R}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[SparkSparseMatrix[T]], Rep[AbstractVector[T] => AbstractVector[R] @uncheckedVariance]) forSome {type T; type R}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
  ...
  object SparkDenseMatrixMethods {
    ...
    object mapBy {
      def unapply(d: Def[_]): Option[(Rep[SparkDenseMatrix[T]], Rep[AbstractVector[T] => AbstractVector[R] @uncheckedVariance]) forSome {type T; type R}] = d match {
        case MethodCall(receiver, method, Seq(f, _*), _) if receiver.elem.isInstanceOf[SparkDenseMatrixElem[_]] && method.getName == "mapBy" =>
          Some((receiver, f)).asInstanceOf[Option[(Rep[SparkDenseMatrix[T]], Rep[AbstractVector[T] => AbstractVector[R] @uncheckedVariance]) forSome {type T; type R}]]
        case _ => None
      }
      def unapply(exp: Exp[_]): Option[(Rep[SparkDenseMatrix[T]], Rep[AbstractVector[T] => AbstractVector[R] @uncheckedVariance]) forSome {type T; type R}] = exp match {
        case Def(d) => unapply(d)
        case _ => None
      }
    }
