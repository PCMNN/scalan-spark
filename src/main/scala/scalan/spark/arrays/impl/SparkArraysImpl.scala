...
// Abs -----------------------------------
trait SparkArraysAbs extends ScalanCommunityDsl with SparkArrays {
  self: SparkDsl =>
  ...
  abstract class SparkArrayElem[A, From, To <: SparkArray[A]](iso: Iso[From, To]) extends ViewElem[From, To]()(iso) {
    override def convert(x: Rep[Reifiable[_]]) = convertSparkArray(x.asRep[SparkArray[A]])
    def convertSparkArray(x : Rep[SparkArray[A]]): Rep[To]
  }
  ...
  class RDDArrayElem[A:Elem](iso: Iso[RDDArrayData[A], RDDArray[A]]) extends SparkArrayElem[A, RDDArrayData[A], RDDArray[A]](iso) {
    def convertSparkArray(x: Rep[SparkArray[A]]) = // Converter is not generated by meta
!!!("Cannot convert from SparkArray to RDDArray: missing fields List(rdd)")
  }
...
// Seq -----------------------------------
trait SparkArraysSeq extends SparkArraysDsl with ScalanCommunityDslSeq {
  self: SparkDslSeq =>
...
// Exp -----------------------------------
trait SparkArraysExp extends SparkArraysDsl with ScalanCommunityDslExp {
  self: SparkDslExp =>
