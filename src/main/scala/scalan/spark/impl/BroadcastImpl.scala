...
trait BroadcastsAbs extends ScalanCommunityDsl with Broadcasts {
  ...
  class SBroadcastImplElem[A:Elem](iso: Iso[SBroadcastImplData[A], SBroadcastImpl[A]]) extends SBroadcastElem[A, SBroadcastImplData[A], SBroadcastImpl[A]](iso) {
    def convertSBroadcast(x: Rep[SBroadcast[A]]) = // Converter is not generated by meta
!!!("Cannot convert from SBroadcast to SBroadcastImpl: missing fields List(wrappedValueOfBaseType)")
  }
